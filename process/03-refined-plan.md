# Refined Plan (post-review, approved)

> Workshop artifact: this is the FINAL plan after the user's review round.
> Frozen snapshot at the moment of approval — do not edit further.
> The living reference document for the build is `/PLAN.md` at repo root.
> The pre-review version is `01-initial-plan.md`; the rejection round is `02-rejection.md`.
...
This plan reflects the user's review revisions: (1) raw transcript surfaced on the detail page, (2) `meetings_with_stats` view committed into the schema, (3) Supabase email-confirmation disabled with signup-error surfacing, (4) model locked to `claude-sonnet-4-6`.
